...
<div class="max-w-xl mx-auto">
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h2 class="text-xl font-bold text-gray-900">Tambah Pengguna</h2>
            <p class="text-sm text-gray-500 font-medium">Buat akun pengguna baru (Administrator atau Auditor).</p>
        </div>
    ...
    <div class="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100">
        <form method="POST" action="{{ route('user.store') }}" class="space-y-6">
            ...
            <div>
                ...
                <input id="nip" name="nip" type="text" value="{{ old('nip','') }}" placeholder="Masukkan NIP (opsional)"
                    class="block w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-blue-600 focus:bg-white transition-all">
            </div>
            ...
            <div>
                ...
                <input id="email" name="email" type="email" value="{{ old('email','') }}" required placeholder="email@inspektorat.go.id"
                    class="block w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-blue-600 focus:bg-white transition-all">
            </div>
