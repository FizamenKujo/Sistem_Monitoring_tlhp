...
<div class="max-w-4xl mx-auto">
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h2 class="text-xl font-bold text-gray-900">Edit Temuan</h2>
            <p class="text-sm text-gray-500 font-medium">Ubah data temuan pemeriksaan yang telah dibuat.</p>
        </div>
        <a href="{{ route('temuan.index') }}" class="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-xl hover:bg-gray-55 transition-colors shadow-sm font-medium">
    ...
    <div class="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100">
        <form method="POST" action="{{ route('temuan.update', $temuan) }}" class="space-y-6">
            @csrf 
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <select id="lhp_id" name="lhp_id" required
                        ...
                        @foreach($lhps as $l)
                        <option value="{{ $l->id }}" {{ (string)old('lhp_id', $temuan->lhp_id) === (string)$l->id ? 'selected' : '' }}>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="uraian" name="uraian" rows="3" required placeholder="Tuliskan uraian temuan pemeriksaan..."
                        class="block w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-blue-600 focus:bg-white transition-all">{{ old('uraian', $temuan->uraian) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <select id="kategori" name="kategori" required
                        ...
                        <option value="{{ $cat }}" {{ old('kategori', $temuan->kategori) === $cat ? 'selected' : '' }}>
                            {{ $cat }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input id="nilai_display" type="text" placeholder="0"
                        class="block w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-blue-600 focus:bg-white transition-all font-medium">
                    <input type="hidden" name="nilai" id="nilai_raw" value="{{ old('nilai', $temuan->nilai ? (float)$temuan->nilai : '') }}">
                </div>
            </div>
