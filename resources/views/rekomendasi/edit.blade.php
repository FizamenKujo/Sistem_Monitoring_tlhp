...
<div class="max-w-2xl mx-auto">
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h2 class="text-xl font-bold text-gray-900">Edit Rekomendasi</h2>
            <p class="text-sm text-gray-500 font-medium">Ubah rincian rekomendasi tindak lanjut.</p>
        </div>
    ...
    <div class="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100">
        <form method="POST" action="{{ route('rekomendasi.update', $rekomendasi) }}" class="space-y-6">
            @csrf 
            @method('PUT')
            ...
            <div>
                ...
                <select id="lhp_id" name="lhp_id" required
                    ...
                    @foreach($lhps as $l)
                    <option value="{{ $l->id }}" {{ (string)old('lhp_id', $rekomendasi->lhp_id) === (string)$l->id ? 'selected' : '' }}>
            ...
            <div>
                ...
                <input id="ditujukan_kepada" name="ditujukan_kepada" type="text" required value="{{ old('ditujukan_kepada', $rekomendasi->ditujukan_kepada) }}"
            ...
            <div>
                ...
                <textarea id="uraian_rekomendasi" name="uraian_rekomendasi" rows="4" required placeholder="Masukkan uraian rekomendasi..."
                    class="block w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-blue-600 focus:bg-white transition-all">{{ old('uraian_rekomendasi', $rekomendasi->uraian_rekomendasi) }}</textarea>
            </div>

            <!-- Target Waktu -->
            <div>
                <label for="target_waktu" class="block text-sm font-semibold text-gray-700 mb-1.5">Target Waktu Penyelesaian</label>
                <input id="target_waktu" name="target_waktu" type="date" value="{{ old('target_waktu', optional($rekomendasi->target_waktu)->format('Y-m-d')) }}"
                    class="block w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-blue-600 focus:bg-white transition-all font-medium">
            </div>

            <!-- Form Actions -->
            <div class="flex items-center justify-end gap-3 border-t border-gray-100 pt-6">
                <a href="{{ route('lhp.show', $rekomendasi->lhp_id) }}" class="px-5 py-2.5 text-sm font-semibold text-gray-700 hover:bg-gray-100 rounded-xl transition-colors">
                    Batal
                </a>
