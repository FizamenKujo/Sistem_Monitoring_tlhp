...
<div class="max-w-3xl mx-auto">
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h2 class="text-xl font-bold text-gray-900">Edit LHP</h2>
            <p class="text-sm text-gray-500 font-medium">Ubah data Laporan Hasil Pemeriksaan (LHP).</p>
        </div>
    ...
    <div class="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100">
        <form method="POST" action="{{ route('lhp.update', $lhp) }}" enctype="multipart/form-data" class="space-y-6">
            @csrf 
            @method('PUT')
            ...
            <div>
                ...
                <input id="no_lhp" name="no_lhp" type="text" value="{{ old('no_lhp', $lhp->no_lhp) }}" required
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input id="tanggal" name="tanggal" type="date" value="{{ old('tanggal', optional($lhp->tanggal)->format('Y-m-d')) }}" required
                        class="block w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-blue-600 focus:bg-white transition-all">
                </div>
                ...
                <div>
                    ...
                    <input id="batas_tindak_lanjut" name="batas_tindak_lanjut" type="date" value="{{ old('batas_tindak_lanjut', optional($lhp->batas_tindak_lanjut)->format('Y-m-d')) }}"
                        class="block w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-blue-600 focus:bg-white transition-all">
                </div>
            </div>
            ...
            <div>
                ...
                <select id="auditi_id" name="auditi_id" required
                    ...
                    <option value="{{ $au->id }}" {{ (string)old('auditi_id', $lhp->auditi_id) === (string)$au->id ? 'selected' : '' }}>
                        {{ $au->nama }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input id="jenis_pemeriksaan" name="jenis_pemeriksaan" type="text" value="{{ old('jenis_pemeriksaan', $lhp->jenis_pemeriksaan) }}" placeholder="mis. PDTT, Reguler"
                        class="block w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-blue-600 focus:bg-white transition-all">
                </div>
                ...
                <div>
                    ...
                    <input id="periode" name="periode" type="text" value="{{ old('periode', $lhp->periode) }}" placeholder="Tahun Anggaran"
                        class="block w-full px-4 py-2.5 bg-gray-50 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-600 focus:border-blue-600 focus:bg-white transition-all">
                </div>
            </div>
            ...
            <div>
                ...
                @if($lhp->file_pdf)
                <div class="mb-2 text-xs font-semibold text-blue-600 flex items-center gap-1.5">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z"/>
                    </svg>
                    <span>File Terunggah: <a href="{{ asset('storage/' . $lhp->file_pdf) }}" target="_blank" class="underline hover:text-blue-700">{{ basename($lhp->file_pdf) }}</a></span>
                </div>
                @endif
